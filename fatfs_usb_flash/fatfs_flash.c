...
{
    uint8_t header[8];
    uint16_t sectors[FATFS_FLASH_NUM_FAT_SECTORS]; // map FAT sectors -> flash sectors
} sector_map;

sector_map fs_map;
bool fs_map_needs_written[15];

uint8_t used_bitmap[FATFS_FLASH_NUM_FLASH_SECTORS]; // we will use 256 flash sectors for 2048 fat sectors
...
uint16_t getNextWriteSector()
{
    ...
    { // first try to find a completely free sector
        for (i = 0; i < FATFS_FLASH_NUM_FLASH_SECTORS; i++)
        {
            if (used_bitmap[(i + search_start_pos) % FATFS_FLASH_NUM_FLASH_SECTORS] == 0)
                break;
        }
        if (i < FATFS_FLASH_NUM_FLASH_SECTORS)
        {
            write_sector = (i + search_start_pos) % FATFS_FLASH_NUM_FLASH_SECTORS;
        ...
        { // no completely free sector, just return the first sector with space
            for (i = 0; i < FATFS_FLASH_NUM_FLASH_SECTORS; i++)
            {
                if (used_bitmap[(i + search_start_pos) % FATFS_FLASH_NUM_FLASH_SECTORS] != 0xFF)
                    break;
            }
            write_sector = (i + search_start_pos) % FATFS_FLASH_NUM_FLASH_SECTORS;
        ...
        search_start_pos = (i + search_start_pos) % FATFS_FLASH_NUM_FLASH_SECTORS;
    }
...
{
    memset(used_bitmap, 0, FATFS_FLASH_NUM_FLASH_SECTORS);
    for (int i = 0; i < 15; i++)
        used_bitmap[i] = 0xFF; // first 15 flash sectors used by fs map

    for (int i = 0; i < FATFS_FLASH_NUM_FAT_SECTORS; i++)
    {
...
int flash_fs_mount()
{
    ...
    if (memcmp(fs_map.header, FATFS_FLASH_MAGIC_8_BYTES, 8) != 0)
    {
...
void flash_fs_create()
{
    ...
    strcpy(fs_map.header, FATFS_FLASH_MAGIC_8_BYTES);
    for (int i = 0; i < 15; i++)
...
void flash_read_sector(uint16_t sector, uint8_t offset, void *buffer, uint16_t size)
{
    ...
    uint32_t fs_start = XIP_BASE + FATFS_FLASH_STORAGE_BASE;
...
{
    //  printf("[FS] ERASE: %d\n", sector);
    uint32_t fs_start = FATFS_FLASH_STORAGE_BASE;
