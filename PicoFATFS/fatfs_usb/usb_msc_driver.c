...
#include "fatfs_usb_defines.h"
#include "diskio.h" /* Declarations of disk functions */

#ifndef USE_SD_CARD
#error USE_SD_CARD is not defined !
#endif
...
static bool ejected[__LunCount__] = {false, false};
...
void tud_msc_inquiry_cb(uint8_t lun, uint8_t vendor_id[8], uint8_t product_id[16], uint8_t product_rev[4])
{
    switch (lun)
    {
    case ELun_Internal_Flash:
    {
        const char vid[] = FATFS_FLASH_USB_VID;
        const char pid[] = FATFS_FLASH_USB_PID;
        const char rev[] = FATFS_FLASH_USB_REV;
    ...
    case ELun_SD_Card:
    {
        const char vid[] = FATFS_SD_USB_VID;
        const char pid[] = FATFS_SD_USB_PID;
        const char rev[] = FATFS_SD_USB_REV;
...
int32_t tud_msc_read10_cb(uint8_t lun, uint32_t lba, uint32_t offset, void *buffer, uint32_t bufsize)
{
    ...
    case ELun_Internal_Flash:
        ...
        if (offset != 0)
            printf("ERROR offset is not 0\n");
        if (bufsize != get_lba_size())
            printf("ERROR bufsize mismatch %d <> %d\n", bufsize, get_lba_size());
#endif
        // out of ramdisk
        if (lba >= get_lba_count())
        {
            printf("read10 out of ramdisk: lba=%lu\n", lba);
            return -1;
        }
...
int32_t tud_msc_write10_cb(uint8_t lun, uint32_t lba, uint32_t offset, uint8_t* buffer, uint32_t bufsize)
{
    ...
    case ELun_Internal_Flash:
        ...
        if (lba >= get_lba_count())
        {
            printf("write10 out of ramdisk: lba=%lu\n", lba);
            return -1;
        }
...
int32_t tud_msc_scsi_cb__flash(uint8_t lun, uint8_t const scsi_cmd[16], void *buffer, uint16_t bufsize)
{
...
int32_t tud_msc_scsi_cb__sd_card(uint8_t lun, uint8_t const scsi_cmd[16], void *buffer, uint16_t bufsize)
{
    void const* response = NULL;
    int32_t resplen = 0;
...
int32_t tud_msc_scsi_cb(uint8_t lun, uint8_t const scsi_cmd[16], void *buffer, uint16_t bufsize)
{
    ...
    {
    case ELun_Internal_Flash:
        return tud_msc_scsi_cb__flash(lun, scsi_cmd, buffer, bufsize);
        break;

    case ELun_SD_Card:
        return tud_msc_scsi_cb__sd_card(lun, scsi_cmd, buffer, bufsize);
        break;
